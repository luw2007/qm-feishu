...
| Group mention | Exactly one explicit mention creates one turn | 2026-08-04T06:01:28Z | `84b026` | [x] |
...
On 2026-08-04, QM and the adapter were restarted and remained healthy and ready throughout a fresh group-mention attempt. Bot-authored instructions with redacted message suffixes `0b2ef2` and `2daf64` asked the separately accessible synthetic user to send one native mention containing nonce `qm-feishu-group-1785817394281`; neither instruction is acceptance evidence. No user event arrived during the initial 480-second observation window. The synthetic user later sent message `84b026`: Feishu identified a user sender with exactly one native bot mention and the expected nonce; the adapter observed one `im.message.receive_v1`, accepted the intake after acknowledgement, claimed and acknowledged one final delivery without intake, decode, or send failure; and an authoritative QM replay of the same message idempotency key returned `replayed: true`. The group-mention row records that later passing event.
